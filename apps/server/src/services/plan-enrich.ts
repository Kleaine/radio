  ...
  const tasks = items.map(async (item, index) => {
    ...
    try {
      ...
      const exactMatches = allSongs.filter(s => {
        ...
        return !blockedPattern.test(s.title) &&
      ...
      const exactMatch = exactMatches.find(s => !livePattern.test(s.title)) ?? exactMatches[0] ?? null;
      // 只匹配歌名
      ...
      const titleMatch = titleMatches.find(s => !livePattern.test(s.title)) ?? titleMatches[0] ?? null;
